...
#该模块用来对CWRU原始数据进行采样并分类存储为npy文件，对于1DCNN分类，主要是通过一维卷积核来对数据完成特征提取，所以需要设置样本长度（数据点）,这里我选取2048，每个mat采样100个样本
#随后将所有采样样本数据整合成数据集，保存为npy文件，数据集shape为样本数x样本长度,标签格式样本数x类型标号
