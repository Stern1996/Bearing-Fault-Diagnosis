...
load_model = load_model('stft.h5')
model = Model(inputs=load_model.input, outputs=load_model.get_layer('dense_2').output)
...
for i in range(1000):
    predict = model.predict(img_list[i].reshape(1,96,96,1))
...
plt.legend()
plt.savefig('stft_features_noise5.png')
plt.show()