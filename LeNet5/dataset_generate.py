...
#该模块用来对CWRU原始数据进行采样并分类存储为npy文件，对于LeNet5分类，主要是将数据为图像二维数组来作为一个样本，这里图像尺寸选取64x64,每个mat文件抽取50张图片
#随后将所有采样样本数据整合成数据集，保存为npy文件，数据集shape为样本数x[64x64],标签格式样本数x类型标号
...
path = "./dataset/"
#parameters
img_height = int(input("Please set the height of each image:"))
img_width = int(input("Please set the width of each image:"))
sample_len = img_height*img_width
num_samples = int(input("Please set the number of samples for each mat-file:"))

#采样数据处理，归一化+灰度处理,输入data为列表
...
def samples_concat():
    ...
    # 降维
    all_data = all_data.reshape((-1, img_height,img_width))
